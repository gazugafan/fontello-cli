		...
		install: function(options) {
			...
			return apiRequest(options, function(sessionUrl) {
				...
				if (options.css && options.font) {
					...
					return zipFile.pipe(unzip.Parse()).on('entry', (function(entry) {
						var outputPath, dirName, fileName, pathName, ref, type;
						({
							path: pathName,
							type
						} = entry);
						if (type === 'File') {
							dirName = (ref = path.dirname(pathName).match(/\/([^\/]*)$/)) != null ? ref[1] : '';
							fileName = path.basename(pathName);
							outputPath = path.join(path.dirname(options.config), dirName, fileName);
							return entry.pipe(fs.createWriteStream(outputPath));
						}
					})).on('finish', (function() {
						return console.log('Install complete.\n'.green);
					}));
